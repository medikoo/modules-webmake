...
let contains     = require("es5-ext/array/#/contains")
	...
	return stat(path)(stats => {
		...
		if (stats.isDirectory()) {
			return readdir(path, { depth: Infinity, type: { file: true } })(
				data => data.map(file => resolve(path, file))
			);
		}
		return [];
	});
...
module.exports = function (input, options, cb) {
	...
	promise = parser.readInput(input, options)(path => deferred.map([].concat(options.include || []), path => {
			path = resolve(String(path));
			return filesAtPath(path).invoke("filter", filename => {
			...
			let src = `${ tpl.replace("SEPARATOR", separator)
