...
let startsWith      = require("es5-ext/string/#/starts-with")
...
module.exports = {
	""(t, a, d) {
		...
		t(input, options)(result => {
			...
			a.deep(program.json, { "raz": 0,
"dwa": "trzy",
"pięć": false,
				"cztery": null,
"osiem:": undefined }, "JSON");
	...
	"Other type includes"(t, a, d) {
		let input = `${ pg }/lib/other-type-includes.js`, options = { include: `${ pg }/includes` };
		t = promisify(t);
		t(input, options)(result => {
	...
	Dynamic: {
		Error(t, a, d) {
			...
			t(input)(a.never, e => { a(e.code, "DYNAMIC_REQUIRE"); })
				.done(d, d);
		},
		Ignored(t, a, d) {
			const input = `${ pg }/lib/dynamic.js`;
	...
	"Ignore error on native"(t, a, d) {
		t(`${ pg }/require-native.js`, { ignoreErrors: true }, (err, data) => {
			...
			program = runInNewContext(data, { require });
