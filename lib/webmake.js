...
var path       = require('path')
  ...
  , indent     = require('es5-ext/lib/String/indent').call(null).call
...
Parser = {
	...
	resolveExternal: function (dirname, filename) {
		...
		var name = filename.split('/', 1)[0], main, tree;
		return s2p(function () {
			if (this.modules[name]) {
				return this.modules[name];
			} else {
				main = getRequire(dirname).resolve(name);
				return this.modules[name] = findRoot(main)
				(function (root) {
					this.packages[name] = root;
					return this.modules[name]
						= { ':mainpath:': shiftSame(main, root).slice(1, -3) };
				}.bind(this));
			}
		}.bind(this))
		(function (scope) {
...
modulesToString = function self (module, nest) {
	...
	return Object.keys(module).map(function (name) {
		...
		if (name === ':mainpath:') {
			text += JSON.stringify(module[name]);
