...
readFileData = function (filename, parser, localFilename) {
	return readFile(filename, readFileOpts)(function (code) {
		...
		if (parser.ext['.js'].hasOwnProperty(ext)) {
			// .js Extension
			code = parser.ext['.js'][ext].compile(code, { filename: filename,
		...
		} else if (parser.ext['.json'].hasOwnProperty(ext)) {
			// .json Extension
			code = parser.ext['.json'][ext].compile(code, { filename: filename,
				localFilename: localFilename, sourceMap: parser.sourceMap }).code;
			code = deferred(code)(function (code) {
	...
	resolveLocal: function (fromfile, dirpath, scope, tree, filename, orgFn) {
		...
		resolveFirstTypePath = function (path) {
			return resolveSpecificPath(path, '.js')(null, function () {
				return resolveSpecificPath(path, '.json');
			});
		};
...
getExt = function (name) {
	...
	if (!this.hasOwnProperty(type)) {
		throw new CustomError("Extension '" + name +
...
module.exports = exports = function (/* options */) {
	var options = Object(arguments[0]), ext = { '.js': {}, '.json': {} };
	if (options.ext) {
