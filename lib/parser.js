...
parser = {
	...
	resolveLocal: function (fromfile, dirpath, scope, tree, filename, orgFn) {
		...
		var resolveDirPath, resolvePath, resolveAnyTypePath, resolveExtPath
		...
		resolveDirPath = function (path) {
			return getMain(path)(function (main) {
				...
				return resolveAnyTypePath(resolve(path, 'index'));
			});
		};

		resolveAnyTypePath = function (path) {
			return statP(path + '.js')(function (stats) {
				...
				return statP(path + '.json')(function (stats) {
					...
					return resolveExtPath(path);
				});
			});
		};

		resolveExtPath = function (path) {
		...
		resolvePath = function (path, forceIndex) {
			var ext = extname(path).toLowerCase();
			if ((ext === '.js') || (ext === '.json')) {
				return statP(path)(function (stats) {
					if (stats.isFile()) return path;
					return resolveAnyTypePath(path)(null, function (err) {
						if (stats.isDirectory()) {
							if (forceIndex) return resolveAnyTypePath(resolve(path, 'index'));
							return resolveDirPath(path);
						}
						throw err;
					});
				}, function () { return resolveAnyTypePath(path); });
			}
			return resolveAnyTypePath(path)(null, function (err) {
				return statP(path)(function (stats) {
					if (stats.isDirectory()) {
