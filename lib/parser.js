...
isOptional = (function () {
	...
	return function (src, point, pathLength) {
		return pre.test(src.slice(0, point - 1)) &&
			post.test(src.slice(point + pathLength - 1));
	};
}());
...
parseDependencies = function (text, filename, ignoreErrors) {
	...
	try {
		...
		throw customError(e.message + " in " + filename, 'AST_ERROR',
			{ origin: e });
	}
...
parser = {
	readInput: function (input) {
		...
		return findRoot(input)(function (root) {
			...
			dirs = input.slice(root.length + 1);
	...
	readStreamInput: function (input, root) {
		var content = '', def = deferred(), scope, name, index = 0;
		statP.clear();
		getMain.clear();
		name = ':stdin:';
		if (!this.modules[name]) {
		...
		scope = this.modules[name];
		while (scope[index]) ++index;

		input.on('data', function (data) {
			content += data;
		});
		input.on('error', def.reject);
		input.on('end', function () {
			readFileContent(content, ':stream.js', this, ':stream.js')(function (data) {
				scope[index] = data.content;
				return deferred.map(data.deps,
					this.resolve.bind(this, ':stream.js', name, scope, []));
			}.bind(this)).aside(function () { def.resolve(name + '/' + index); });
		}.bind(this));
		return def.promise;
	},
	...
	resolve: function (fromfile, dirname, scope, tree, dep) {
		...
		tree = aFrom(tree);
		if (dep.value[0] === '.') {
			return this.resolveLocal(fromfile, dirname, scope, tree, dep);
		}
		...
		return deferred.promisifySync(function () {
			...
			try {
				...
				try {
					...
					if (isOptional(scope[basename(fromfile)],
							dep.point, dep.raw.length)) {
						return;
					}
