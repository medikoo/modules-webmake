...
let aFrom        = require("es5-ext/array/from")
...
isOptional = (function () {
	let pre = /try\s*\{(?:\s*[a-zA-Z][0-9a-zA-Z]*\s*=)?\s*require\(\s*$/
...
getMain = memoize(path => readFile(resolve(path, "package.json"), readFileOpts)(
		content => {
			const main = parse(stripBOM(content)).main;
...
readFileContent = function (code, filename, parser, localFilename) {
	let ext = extname(filename), type, sourceUrl, data;
	...
	if ((ext !== ".js") && (ext !== ".json") && (ext !== ".css") && (ext !== ".html")) {
		...
		// Extension
		if (parser.transform) {
			code = deferred(parser.transform(filename, code))(code => {
				let data;
				if (code == null) {
				...
				code = String(code);
				data = parser.ext[type][ext].compile(code, { filename,
					localFilename,
...
sourceMap: parser.sourceMap,
		...
		if (parser.transform) {
			code = deferred(parser.transform(filename, code))(data => {
				let code;
				if (data == null) {
					throw customError("Provided transform callback must return code string",
						"INVALID_TRANSFORM");
	...
	return deferred(code)(code => {
		...
		return { content: code, deps };
	});
};

readFileData = function (filename, parser, localFilename) {
	return readFile(filename, readFileOpts)(code => readFileContent(code, filename, parser, localFilename));
};
...
modulesToString = function self(nest, parser) {
	const format = parser.prettyOutput ? indent : getThis;
...
parser = {
	readInput(input/* , options*/) {
		let scope, path, tree = [], options = Object(arguments[1]), stream = options.stream
		  , content, def;
		...
		return findRoot(input)(root => {
			...
			return def.promise(content => readFileContent(content, name, this, name)(data => {
					scope[name] = data.content;
					return deferred.map(data.deps, this.resolve.bind(this, input, dirname(input), scope, []));
				}));
		})(() => path.slice(0, -extname(path).length).replace(/\\/g, "/"));
	},
	...
	resolveLocal(fromfile, dirpath, scope, tree, dep, orgFn, lScope) {
		let filename = dep.localFilename || dep.value, extNames = this.extNames
		...
		resolveDirPath = function (path) {
			return getMain(path)(main => resolvePath(resolve(path, main), true), err => resolveFirstTypePath(resolve(path, "index")));
		};

		resolveFirstTypePath = function (path) {
			return resolveSpecificPath(path, ".js")(null, () => resolveSpecificPath(path, ".json")(null, () => resolveSpecificPath(path, ".css")(null, () => resolveSpecificPath(path, ".html"))));
		};
		...
		resolveExtTypePath = function (path, ext) {
			...
			return deferred.some(exts, ext => statP(path + ext)(stats => {
					if (stats.isFile()) {
						path += ext;
						return true;
					}
		...
		resolvePath = (function () {
			const resolvePath = function (path, forceIndex) {
				return resolveFirstTypePath(path)(null, err => statP(path)(stats => {
			...
			return function (path, forceIndex) {
				...
				if ((ext === ".js") || (ext === ".json") || (ext === ".css") || (ext === ".html")) {
					return resolveSpecificPath(path.slice(0, -ext.length), ext)(null,
						err => resolvePath(path, forceIndex));
				}
		...
		return init(resolve(dirpath, filename))(filename => {
			let dir, tokens, index, result
			...
			if (this.ignored.has(filename)) return filename;
	...
	resolveExternal(fromfile, dirname, scope, dep) {
		// console.log("RE", dep.value);
		let org = dep.value, filename = join(dep.value)
		...
		return deferred.promisifySync(() => {
			...
			const promise = this.modules[name] = findRoot(path)(root => {
				...
				return getMain(root)(main => {
					module[":mainpath:"] = main;
					return module;
				}, module);
			});
			promise.aside(module => { this.modules[name] = module; });
			return promise;
...
module.exports = exports = function (/* options */) {
	let options = Object(arguments[0])
	  , ext = { ".js": {}, ".json": {}, ".css": {}, ".html": {} }, ignored;
	if (options.ext) {
		if (isArray(options.ext)) options.ext.forEach(getExt, ext);
		else getExt.call(ext, options.ext);
	}
	ignored = options.ignore ? new Set(aFrom(options.ignore)) : new Set();
