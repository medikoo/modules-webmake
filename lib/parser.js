...
parseDependencies = function (text) {
    return bloodhound(text, true).map(function(matchingNode) {
        if (matchingNode.type !== "string") {
            throw new TypeError("Not supported require call: require(" + matchingNode.value + ")");
        }
        return (matchingNode.value.slice(-3) === '.js') ? matchingNode.value.slice(0, -3) : matchingNode.value;
    });
};
	...
	return keys.map(function (name) {
		...
		if (name === ':mainpath:') {
			...
			text += 'function (exports, module, require) {\n' +
				indent.call(module[name], nest + 2) + indent.call('}', nest + 1);
		}
		return text;
	}).join(',\n');
};
