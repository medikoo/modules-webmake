...
if (sep == null) {
	throw new Error(
		"Unsupported Node version. Please upgrade," + " Webmake needs at least Node v0.8"
	);
}
	...
	if (ext !== ".js" && ext !== ".json" && ext !== ".css" && ext !== ".html") {
		...
		if (parser.transform) {
			code = deferred(parser.transform(filename, code))(code => {
				...
				code = String(code);
				data = parser.ext[type][ext].compile(code, {
					filename,
					localFilename,
				...
				if (parser.sourceMap && data.sourceMap) {
					code += `//# sourceMappingURL=data:application/json;base64,${ new Buffer(
						data.sourceMap
					).toString("base64") }\n`;
				}
				return code;
...
readFileData = function (filename, parser, localFilename) {
	return readFile(filename, readFileOpts)(code =>
		readFileContent(code, filename, parser, localFilename));
};
...
modulesToString = function self(nest, parser) {
	...
	return keys(this)
		.sort()
		.map(function (name) {
			let current = this[name], text = `${ format.call(stringify(name), "\t", nest + 1) }: `;
...
parser = {
	readInput(input /* , options*/) {
		...
		return findRoot(input)(root => {
			...
			return def.promise(content =>
				readFileContent(content, name, this, name)(data => {
	...
	resolveLocal(fromfile, dirpath, scope, tree, dep, orgFn, lScope) {
		let filename = dep.localFilename || dep.value
		  , extNames = this.extNames
		  , resolveDirPath
		  , resolvePath
		  , resolveFirstTypePath
		  , resolveExtTypePath
		  , resolveSpecificPath
		  , init;
		...
		resolveDirPath = function (path) {
			return getMain(path)(
				...
				err => resolveFirstTypePath(resolve(path, "index"))
			);
		};
		...
		resolvePath = (function () {
			...
			return function (path, forceIndex) {
				...
				if (ext === ".js" || ext === ".json" || ext === ".css" || ext === ".html") {
					return resolveSpecificPath(path.slice(0, -ext.length), ext)(null, err =>
						resolvePath(path, forceIndex));
				}
		...
		return init(resolve(dirpath, filename))(
			filename => {
				let dir, tokens, index, result, name = basename(filename), path = dirname(filename);
	...
	resolveExternal(fromfile, dirname, scope, dep) {
		...
		return deferred.promisifySync(() => {
			...
			if (main === name) {
				...
				try { path = main = require.resolve(name); }
				catch (ignore) {}
				nmSource[name] = cache;
				if (main === name) {
					// No substitute found
...
getExt = function (ext) {
	...
	if (isArray(ext.extension)) {
		ext.extension.forEach(function (name) { this[type][`.${ String(name) }`] = ext; }, this);
	} else {
...
module.exports = exports = function (/* options */) {
	...
	return create(parser, {
		...
		prettyOutput: d(options.prettyOutput == null ? true : Boolean(options.prettyOutput)),
