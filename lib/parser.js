...
var copy         = require('es5-ext/lib/Array/prototype/copy')
  ...
  , extname = path.extname, dirname = path.dirname, join = path.join
  , resolve = path.resolve, sep = path.sep
  , readFileOpts = { encoding: 'utf8' }
  , dotMatch = /^\.+$/
  , readFileData, readFileDataCached, fileExists, dirExists, parseDependencies
  , parser, modulesToString, normalize;
...
parser = {
	...
	resolveLocal: function (fromfile, dirname, scope, tree, filename) {
		...
		return (isDir ? dirExists(filename)(function (exists) {
			...
			else return (scope[name] = this.readFile(filename, name, scope, tree));
		}.bind(this));
	},
